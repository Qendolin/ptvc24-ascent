...
void ScoreScreen::open(ScoreEntry score) {
    opened_ = true;
    score_ = score;
    appearOpacity_.seek(0, true);
    graphProgress_.seek(0, true);
    Game::get().input->setMouseMode(Input::MouseMode::Release);
}

void ScoreScreen::draw_() {
    ...
    if (nk_begin(nk, "score_screen", {0_vw, 0_vh, 100_vw, 100_vh}, NK_WINDOW_NO_SCROLLBAR)) {
        game.tween->step(appearOpacity_);
        ...
        if (nk_group_begin(nk, "scores", NK_WINDOW_NO_SCROLLBAR)) {
            ...
            nk_style_push_color(nk, &nk->style.text.color, nk_rgba_f(1, 1, 1, appearOpacity_.peek()));
            nk_label(nk, "Time", NK_TEXT_ALIGN_RIGHT);
            std::string time_str = formatTimeRaceClock(score_.flight);
            nk_label(nk, time_str.c_str(), NK_TEXT_ALIGN_LEFT);
            nk_style_pop_color(nk);

            nk_style_push_color(nk, &nk->style.text.color, nk_rgba_f(1, 1, 1, appearOpacity_.peek() - 1));
            nk_label(nk, "Penalty", NK_TEXT_ALIGN_RIGHT);
            std::string penalty_str = formatTimeRaceClock(score_.penalty);
            ...
            nk_style_push_color(nk, &nk->style.text.color, nk_rgba_f(1, 1, 1, appearOpacity_.peek() - 3));
            nk_label(nk, "Total", NK_TEXT_ALIGN_RIGHT);
            std::string total_str = formatTimeRaceClock(score_.total);
            ...
            if (appearOpacity_.progress() == 1.0) {
                drawPerformance_();
            }
...
void ScoreScreen::drawPerformance_() {
    ...
    game.tween->step(graphProgress_);
    // background
    nk_fill_rect(canvas, graph_space, 5_dp, nk_rgba_f(1, 1, 1, 0.2f * graphProgress_.peek()));

    auto scissor_rect = graph_space;
    scissor_rect.w *= graphProgress_.peek();
    ...
    // hi score
    if (graphProgress_.progress() == 1.0 && hi_score.valid) {
    ...
    for (int i = 0; i < limited_count; i++) {
        ...
        if (i == 0 && graphProgress_.progress() == 1.0) {
